    ...
    public static ImageAsset ReadFromFile(string path)
    {
        ...
        unsafe
        {
            ...
            fixed (byte* p_encodedData = encodedData)
            {
                ...
                int height = 0;
                int hasAlpha = 0;
                status = webpAnalyze(p_encodedData, (ulong)encodedData.Length, &width, &height, &hasAlpha);
                ThrowInvalidDataIf(status != WebPStatus.OK, $"Failed to analyze image: {status}");

                byte* decoded_data = null;
                ulong decoded_data_size = 0;
                status = webpDecode(p_encodedData, (ulong)encodedData.Length, WebPFormat.RGBA, &decoded_data, &decoded_data_size);
