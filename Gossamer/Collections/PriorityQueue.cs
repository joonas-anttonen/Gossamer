...
public sealed class PriorityQueue<T> : IEnumerable<T> where T : PriorityQueueNode
{
    ...
    {
        ThrowInvalidOperationIf(maxNodes <= 0, "Queue size cannot be smaller than 1");
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static bool HasHigherPriority(T higher, T lower)
    {
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    static bool HasHigherOrEqualPriority(T higher, T lower)
    {
        return higher.Priority <= lower.Priority;
    }
